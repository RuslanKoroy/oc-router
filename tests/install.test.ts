...
describe("installer", () => {
  test("removes conflicting npm plugin id", () => {
    const root = tempRoot()
    ...
    expect(JSON.parse(readFileSync(path, "utf8")).plugin).toEqual(["existing"])
  })
  ...
  test("installs router config, models report, and OpenCode integration", () => {
    ...
    expect(opencodeConfig.plugin ?? []).not.toContain("oc-router")
  })
  ...
  test("sets router as default OpenCode agent while preserving config", () => {
    ...
    expect(config).toMatchObject({ model: "openai/gpt-4o", default_agent: "router", plugin: ["existing"] })
  })
